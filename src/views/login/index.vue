      ...
      >
        <div w-full flex flex-col items-center>
          ...
          <transition name="fade-slide" mode="out-in">
            <component :is="formComponent[component]" v-model="component" class="w-90%" />
          </transition>
        </div>
      </div>
      <!-- 登录页底部版权 -->
      <el-text size="small" class="py-2.5! fixed bottom-0 text-center">
        Copyright © 2025 youlai.study.tech All Rights Reserved.
        <a href="#" target="_blank">测试备案号</a>
      </el-text>
    </div>
  </div>
</template>
...
const component = ref<LayoutMap>("login"); // 切换显示组件
const formComponent = {
